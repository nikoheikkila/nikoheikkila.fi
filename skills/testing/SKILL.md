...
- **The coverage denominator is an explicit `include` list**: the V8 provider only reports files a test actually imported, so never-imported backend modules silently vanish from the report and inflate the percentage. `vitest.config.ts` therefore lists the measured backend sources (`gatsby/**`, `gatsby-node.ts`, `gatsby-config.ts`, `src/utils/**`, `src/search/index.ts`, `src/components/layout/socialIcons.ts`, `infra/site/worker.ts`). Add new backend files to that list. Because the list is explicit, filtering a run to one test file no longer narrows the report.
- **Prefer stubs and fakes over `vi.fn()`/`vi.mock()`**: unit tests inject hand-written typed stubs and recording fakes through the seams the sources expose (`createOnCreatePages`, `createOnCreateNodes`, `createConfig`, the Worker's `env.SITE`). Shared Gatsby fakes live in `src/__tests__/unit/gatsby/fakes.ts`.
