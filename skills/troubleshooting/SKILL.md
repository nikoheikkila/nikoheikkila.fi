...
-   **Port conflicts**: Development server runs on <http://localhost:8000>. `task typegen` deliberately starts its throwaway dev server on 8999 so it cannot collide with `task dev` or `task serve` — a collision makes gatsby-cli prompt for another port and deadlock on piped stdio.
