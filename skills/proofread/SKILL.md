...
Accept a path to the Markdown file needing review. If the user omitted the path, ask if they intended to review a
specific file or all the files under `content/` directory.

Review the received Markdown content for writing quality across the following criteria:

1. Text must be grammatically correct British English unless different language is specified in the frontmatter.
2. Text must be captivating to the reader and follow a creative prose style.
3. Frontmatter metadata must be valid.
4. Scientific and technical claims made must be accurate and backed up by sources.
5. Content must be optimised for search engines to find easily.
6. All hyperlinks must point to an existing resource.

## Review Checklist

When reviewing content, check all the areas and report findings grouped by category.
