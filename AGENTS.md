...
- **Shell working directory**: Return the shell to the repo root after working in a subdirectory. Hooks and `task` resolve against the current directory — a shell left in `infra/` makes root tooling (vitest, the root Taskfile) fail confusingly.
